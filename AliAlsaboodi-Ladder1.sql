...
select age, name from pets where sex='M'
...
select * from pets where sex='M' and species='dog' or sex='F' and species='cat'
...
select name, age from pets where sex='F' and species = 'cat' order by age desc
...
select * from employees_null where job is null --unknown

-- 18) Who is the newest employee in `employees`? The most senior?
select * from employees order by startdate asc --Newest Roger Connor, Senior Mary Nash
...
select * from employees where (firstname='Robert' or firstname='Lisa' or firstname like 'J%') and job!='Sales'
-- 
-- ## Column Operations
...
select firstname  ||', '||  lastname as fullname, round(cast(salary as NUMERIC)/1.1,2) as salary_eu from employees
...
select firstname  ||', '||  lastname as fullname, round(cast(salary as NUMERIC)/1.1,2) as salary_eu, substr(startdate,1,4) as startyear from employees


-- 27) Repeat the above problem, but instead of using `SUBSTR()`, use `STRFTIME()`.
select firstname  ||', '||  lastname as fullname, round(cast(salary as NUMERIC)/1.1,2) as salary_eu, strftime('%Y', startdate) as startyear from employees
...
select firstname  ||', '||  lastname as fullname, substr(startdate,1,4) as startyear, printf('$%,.0f',salary) as salary from employees
...
select *, case when job='Sales' then 'Yes' else 'No' end as bonus from employees
...
select *, case when job='Sales' and salary>= 100000 then 'Yes' else 'No' end as bonus from employees
...
select *, case when job='Sales' and salary >100000 then printf('$%,.0f',salary*1.1) when job='Sales' and salary <100000 then printf('$%,.0f',salary*1.05) when job='Administrators' then printf('$%,.0f',salary*1.05) else 'Not eligible' end as target_comp from employees